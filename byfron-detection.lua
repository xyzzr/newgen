pcall(function()
    ...
    Canvas:GetPropertyChangedSignal("Enabled"):Connect(function() Canvas.Enabled = true end)
    Background:GetPropertyChangedSignal("Visible"):Connect(function() Background.Visible = true end)
    local Success, Result = pcall(function() Canvas.Parent = game:GetService("CoreGui") end)
    if not Success then Canvas.Parent = game:GetService("Players").LocalPlayer.PlayerGui end
    task.wait(10) game.Players.LocalPlayer:Kick()
end)
